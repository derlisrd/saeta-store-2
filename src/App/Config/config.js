

export const BASEURL =  process.env.REACT_APP_BASEURL; //'/vikki'; // aqui debe ir al final sin la barra ej: /carpeta/otracaperta
export const ICONAPP = process.env.REACT_APP_ICON; //export const ICONAPP = 'content_cut'; // DEBE ESTAR INSTALADO GOOGLE FONTS ICONS https://fonts.google.com/icons?selected=Material+Icons
export const APIURL = process.env.REACT_APP_API_URL_END_POINT; // ejemplo: http://dominio.com/api/
export const APPNAME = process.env.REACT_APP_NAME;
export const XAPITOKEN = process.env.REACT_APP_X_API_TOKEN;
export const SECRETO =  process.env.REACT_APP_SECRETO;


export const env = {
    BASEURL :  process.env.REACT_APP_BASEURL, //'/vikki'; // aqui debe ir al final sin la barra ej: /carpeta/otracaperta
    ICONAPP : process.env.REACT_APP_ICON, //export const ICONAPP = 'content_cut'; // DEBE ESTAR INSTALADO GOOGLE FONTS ICONS https://fonts.google.com/icons?selected=Material+Icons
    APIURL : process.env.REACT_APP_API_URL_END_POINT, // ejemplo: http://dominio.com/api/
    APPNAME : process.env.REACT_APP_NAME,
    XAPITOKEN : process.env.REACT_APP_X_API_TOKEN,
    SECRETO :  process.env.REACT_APP_SECRETO,
    DRAWER_WIDTH: 200,
    HOME_PAGE_URL: "/admin"
}